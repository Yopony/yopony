...
{
    // Convert yopony:// to yopony:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("yopony://"))
    {
        uri.replace(0, 12, "yopony:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "YoPony.lnk";
}
...
{
    return GetAutostartDir() / "yopony.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("YoPony-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  yopony-qt [" + tr("command-line options") + "]                     " + "\n";
